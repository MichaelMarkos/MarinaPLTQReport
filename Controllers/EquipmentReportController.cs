...
public class EquipmentReportController : ControllerBase
{
    ...
    public async Task<IActionResult> PostElevatorReport()
    {
        ...
        var report = new Elevator
        {
            ...
            resizableSquarewidth = int.TryParse(form["resizableSquarewidth"], out var resizableSquarewidth)
                ? resizableSquarewidth
                : 0,
            resizableSquareHeight = int.TryParse(form["resizableSquareHeight"], out var resizableSquareHeight)
                ? resizableSquareHeight
                : 0,
    ...
    [HttpPost("editElevatorReport")]
    [RequestSizeLimit(20_000_000)] // 20 MB
    public async Task<IActionResult> editElevatorReport()
    {
        var form = await Request.ReadFormAsync();

        var oldReport = await _db.Elevator.FirstOrDefaultAsync(x => x.Id == int.Parse(form["reportId"]));
        ...
        oldReport.garagsNum=int.TryParse(form ["garagsNum"] , out var garagsNum) ? garagsNum : 0;
        oldReport.heightShape=int.TryParse(form ["height"] , out var height) ? height : 0;
        oldReport.radiusShape=int.TryParse(form ["radius"] , out var radius) ? radius : 0;
        oldReport.floors=int.TryParse(form ["floors"] , out var floors) ? floors : 0;
        oldReport.foundationHeight=int.TryParse(form ["foundationHeight"] , out var foundationHeight) ? foundationHeight : 0;
        oldReport.resizableSquarewidth=int.TryParse(form ["resizableSquarewidth"] , out var resizableSquarewidth)
           ? resizableSquarewidth
           : 0;
        oldReport.resizableSquareHeight=int.TryParse(form ["resizableSquareHeight"] , out var resizableSquareHeight)
           ? resizableSquareHeight
           : 0;

        oldReport.wellStatus=form ["wellStatus"];
        oldReport.capinaStatus=form ["capinaStatus"];
        oldReport.directionWidth=int.TryParse(form ["directionWidth"] , out var directionWidth) ? directionWidth : 0;
        oldReport.directionHeight=int.TryParse(form ["directionHeight"] , out var directionHeight) ? directionHeight : 0;
        oldReport.liftWidth=int.TryParse(form ["liftWidth"] , out var liftWidth) ? liftWidth : 0;
        oldReport.rightWidth=int.TryParse(form ["rightWidth"] , out var rightWidth) ? rightWidth : 0;
        oldReport.centerWidth=int.TryParse(form ["centerWidth"] , out var centerWidth) ? centerWidth : 0;
        oldReport.capinaHeight=int.TryParse(form ["capinaHeight"] , out var capinaHeight) ? capinaHeight : 0;


        oldReport.floorHeights=form ["floorHeights"];
        oldReport.garagsHeights=form ["garagsHeights"];
        oldReport.shapeType=form ["shapeType"];
        oldReport.typeElevator=form ["typeElevator"];
        oldReport.workRequied=form ["workRequied"];
        oldReport.doorDirections=form ["doorDirections"];
        oldReport.doortwoDirections=form ["doortwoDirections"];
        oldReport.garagsDirections=form ["garagsDirections"];
        oldReport.garagstwoDirections=form ["garagstwoDirections"];
        oldReport.twoDirectionFlag=bool.Parse(form ["twoDirectionFlag"]);

        oldReport.Notes=form ["notes"];
        oldReport.PhoneNum=form ["phoneNum"];
        // Signatures (paths to be saved after upload)
        //  ClientSignaturePath = form["clientSignaturePath"],
        // TechSignaturePath = form["techSignaturePath"],
        ...
        foreach(var file in form.Files)
        {
            ...
            {
                case "wellImage":
                    oldReport.WellImagePath=relativePath;
                    break;

                case "directionImage":
                    oldReport.DirectionImagePath=relativePath;
                    break;

                case "resizableImage":
                    oldReport.ResizableImagePath=relativePath;
                    break;
        ...
        if(oldReport.shapeType=="square")
        {
            oldReport.radiusShape=null;
        }
        else if(oldReport.shapeType=="circle")
        {
            oldReport.heightShape=null;
            oldReport.widthShape=null;
        }

        _db.Elevator.Update(oldReport);
        await _db.SaveChangesAsync();

        return Ok(new { success = true , oldReport.Id , message = "Report saved successfully." });
    }
    ...
    [HttpGet("GetelevatorDetails")]
    public async Task<IActionResult> GetelevatorDetails(int reportId)
    {
    ...
    public async Task<IActionResult> editsafetyReport([FromForm] IFormCollection request)
    {
        try
        {
            ...
            var Id = int.Parse(request["Id"]);
            var sitereport = _db.SafetyReport.FirstOrDefault(r => r.Id == Id);
            ...
            sitereport.Date=DateTime.TryParse(request ["date"] , out var parsedDate) ? parsedDate : DateTime.Now;
            sitereport.PhoneNum=request ["phoneNum"];
            sitereport.TeamNum=int.Parse(request ["teamNum"]);
            sitereport.TeamLeaderName=request ["teamLeaderName"];
            sitereport.TeamLeaderNum=int.Parse(request ["teamLeaderNum"]);
            sitereport.CreatedAt=DateTime.Now;




            _db.SafetyReport.Update(sitereport);
            await _db.SaveChangesAsync();

            _db.SafetyItemsReport.RemoveRange(_db.SafetyItemsReport.Where(x => x.SafetyReportId==Id));
            ...
            foreach(var item in Items)
            {
                if(item.faultFlag==true||item.CorrectiveActionFlag==true)
                {
                    ...
                    {
                        SafetyItemsId = item.CheckingItemId,
                        fault = item.Fault,
                        CorrectiveAction = item.CorrectiveAction,
                        faultFlag = item.faultFlag,
                        CorrectiveActionFlag = item.CorrectiveActionFlag,
                        //CreatedAt = DateTime.Now,
                        SafetyReportId = sitereport.Id
                    };
                    //if(string.IsNullOrEmpty(item.Fault)&&string.IsNullOrEmpty(item.CorrectiveAction)&&item.faultFlag==true&&item.CorrectiveActionFlag==item.faultFlag==true)
                    //    continue;
    ...
    public IActionResult GetReportPdf23(int Id)
    {
        ...
        var ReasonOfVisitJson = (reportDb.Installation != 0 ? reportDb.Installation + " Installation ," : "") +
                                ...
                                (reportDb.PeriodicMaintenance != 0
                                    ? reportDb.PeriodicMaintenance + " PeriodicMaintenance ,"
                                    : "") +
                    ...
                    .Column(col =>
                    {
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem().Text($"Report # : {reportDb.ReportNumber}").FontFamily("Cairo")
                                .FontSize(12);
                            row.RelativeItem().Text($"Invoice # : {reportDb.InvoiceNumber}").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        ...
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem()
                                .Text($"Model : {reportDb.ModelMarnia}    {reportDb.ModelMarniaHireOrSale}")
                        ...
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem()
                                .Text(
                                    $"Model : {reportDb.Model}                                S.N:{reportDb.SerialNumber}")
                        ...
                        {
                            row.Spacing(20); // المسافة بين العناصر
                            row.RelativeItem().Text($"Warranty : {reportDb.WarrantyStatus} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        ...
                        {
                            row.Spacing(20); // المسافة بين العناصر
                            row.RelativeItem().Text($"Reason Of Visit : {ReasonOfVisitJson} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        col.Item().Row(row =>
                        {
                        ...
                        {
                            row.Spacing(20); // المسافة بين العناصر
                            row.RelativeItem().Text($"Received payment : {reportDb.receivedPayment}  QR")
                                .FontFamily("Cairo").FontSize(12);
                        });
                        col.Item().LineHorizontal(1)
                            .LineColor(Colors.Grey.Lighten2);
                        ...
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem()
                                .Text(
                                    $"All Workings have been reviewed at the site and there is no damaged happened due the installation, UN installation and maintenance and the machine is working in a good condition. Please check before leaving.\r\nتمت مراجعه كافه الاعمال بالموقع ولا توجد أى خسائر ناتجة عن أعمال الفك أو التركيب أو الصيانة التى تمت بالموقع والمعدة تعمل بحاله جيدة وشركة مارينا غير مسئوله عن اى ضرر يتم أكتشافة بعد مغادرة الموقع لذلك يرجى مراجعة مكان التركيب جيدا قبل مغادرة الفنيين\r\n ")
                                .FontFamily("Cairo").FontSize(12);
                        });
                        col.Item().Row(row =>
                        {
                            row.Spacing(20); // المسافة بين العناصر
                            row.RelativeItem().Text($"PhoneNum. : {reportDb.PhoneNum} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        ...
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem().Text($"Marina REP. : {reportDb.TechName} ").FontFamily("Cairo")
                                .FontSize(12);
                            //row.RelativeItem().Text($"PhoneNum. : {reportDb.PhoneNum} ").FontFamily("Cairo").FontSize(12);
                            row.RelativeItem().Text($"Site REP. : {reportDb.ClientName} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                    ...
                    .Column(col =>
                    {
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem().Text($"Report # : {SiteReportDb.ReportNumber}").FontFamily("Cairo")
                                .FontSize(12);
                            row.RelativeItem().Text($"Invoice # : {SiteReportDb.InvoiceNumber}").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        ...
                        col.Item().Scale(0.85f).Table(table =>
                        {
                            ...
                            table.Header(header =>
                            {
                                header.Cell().Border(1).Background("#f0f0f0").Padding(1).AlignMiddle().AlignCenter()
                                    .Text("Items").FontFamily("Cairo").FontSize(8).Bold();
                                header.Cell().Border(1).Background("#f0f0f0").Padding(1).AlignMiddle().AlignCenter()
                                    .Text("Review").FontFamily("Cairo").FontSize(8).Bold();
                                header.Cell().Border(1).Background("#f0f0f0").Padding(1).AlignMiddle().AlignCenter()
                                    .Text("Fault").FontFamily("Cairo").FontSize(8).Bold();
                                header.Cell().Border(1).Background("#f0f0f0").Padding(1).AlignMiddle().AlignCenter()
                                    .Text("Corrective").FontFamily("Cairo").FontSize(8).Bold();
                                header.Cell().Border(1).Background("#f0f0f0").Padding(1).AlignMiddle().AlignCenter()
                                    .Text("Fault").FontFamily("Cairo").FontSize(8).Bold();
                                header.Cell().Border(1).Background("#f0f0f0").Padding(1).AlignMiddle().AlignCenter()
                                    .Text("Corrective").FontFamily("Cairo").FontSize(8).Bold();
                            ...
                            {
                                table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4).AlignMiddle()
                                    .AlignCenter()
                        ...
                        {
                            row.Spacing(15); // المسافة بين العناصر
                            row.RelativeItem().Text($"PhoneNum. : {SiteReportDb.PhoneNum} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        ...
                        {
                            row.Spacing(50); // المسافة بين العناصر
                            row.RelativeItem().Text($"Marina REP. : {SiteReportDb.TechName} ").FontFamily("Cairo")
                                .FontSize(12);
                            //row.RelativeItem().Text($"PhoneNum. : {reportDb.PhoneNum} ").FontFamily("Cairo").FontSize(12);
                            row.RelativeItem().Text($"Site REP. : {SiteReportDb.ClientName} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        // صورة داخل المحتوى كمثال إضافي
                        col.Item().Layers(layers =>
            ...
            .Table(table =>
            {
                ...
                foreach (var item in checkingItems)
                {
                    ...
                    table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4)
                            ...
                            {
                                row.Spacing(10); // المسافة بين العناصر
                                row.RelativeItem().Text($"Team Num. : {SiteReportDb.TeamNum}").FontFamily("Cairo")
                                    .FontSize(10);
                                row.Spacing(20);
                                row.RelativeItem().Text($"Leader Name  : {SiteReportDb.TeamLeaderName}").FontFamily("Cairo")
                                    .FontSize(10);
                                row.RelativeItem().Text($"Leader Num. {SiteReportDb.TeamLeaderNum}").FontFamily("Cairo")
                                    .FontSize(10);
                            });
    ...
    public IActionResult GetDeliveryReportPdf(int Id)
    {
        ...
        var DeliveryReport = new DeliveryReportDetailDto
        {
            ...
            ClientSignaturePath = DeliveryReportDb.ClientSignaturePath != null
                ? baseUrl + DeliveryReportDb.ClientSignaturePath
                : null,
            TechSignaturePath = baseUrl + DeliveryReportDb.TechSignaturePath != null
                ? baseUrl + DeliveryReportDb.TechSignaturePath
                : null,
            ...
            container.Page(page =>
            {
                ...
                page.Content()
                    .Column(col =>
                    {
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem().Text($"Report # : {DeliveryReportDb.ReportNumber}").FontFamily("Cairo")
                                .FontSize(12);
                            row.RelativeItem().Text($"Invoice # : {DeliveryReportDb.InvoiceNumber}").FontFamily("Cairo")
                                .FontSize(12);
                        });
                 ...
                 && item.Description.EndsWith("M");
                        ...
                        {
                            row.Spacing(50); // المسافة بين العناصر
                            row.RelativeItem().Text($"Marina REP. : {DeliveryReportDb.TechName} ").FontFamily("Cairo")
                                .FontSize(12);
                            //row.RelativeItem().Text($"PhoneNum. : {reportDb.PhoneNum} ").FontFamily("Cairo").FontSize(12);
                            row.RelativeItem().Text($"Site REP. : {DeliveryReportDb.ClientName} ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        // صورة داخل المحتوى كمثال إضافي
                        col.Item().Layers(layers =>
                    ...
                    .Column(col =>
                    {
                        ...
                        col.Item().Row(row =>
                        {
                            ...
                            row.RelativeItem().Text($"Company Name : {elevatorReportDb.CompanyName}").FontFamily("Cairo")
                                .FontSize(12);
                            row.Spacing(60);
                            row.RelativeItem().Text($"PO number # : {elevatorReportDb.ClientName}").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        ...
                        {
                            row.Spacing(10); // المسافة بين العناصر
                            row.RelativeItem().AlignCenter().Text("مراجعة المواصفات الفنية المطلوبة").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        col.Spacing(5);
                        ...
                        col.Item().Scale(0.87f).Table(table =>
                        {
                            ...
                            foreach (var item in checkingItems.Where(x => x.Type == "مراجعة المواصفات الفنية المطلوبة"))
                            {
                                table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4)
                                    .Text(item.Item).FontFamily("Cairo").FontSize(9);
                        ...
                        {
                            row.Spacing(10); // المسافة بين العناصر
                            row.RelativeItem().AlignCenter().Text("مراجعة الديكور والتشطيب").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        col.Spacing(5);
                        ...
                        col.Item().Scale(0.87f).Table(table =>
                        {
                            ...
                            foreach (var item in checkingItems.Where(x => x.Type == "مراجعة الديكور والتشطيب"))
                            {
                                table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4)
                                    .Text(item.Item).FontFamily("Cairo").FontSize(9);
                        ...
                        {
                            row.Spacing(10); // المسافة بين العناصر
                            row.RelativeItem().AlignCenter().Text("فحص الأضرار قبل التسليم").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        col.Spacing(5);
                        ...
                        col.Item().Scale(0.87f).Table(table =>
                        {
                            ...
                            foreach (var item in checkingItems.Where(x => x.Type == "فحص الأضرار قبل التسليم"))
                            {
                                table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4)
                                    .Text(item.Item).FontFamily("Cairo").FontSize(9);
                        ...
                        {
                            row.Spacing(10); // المسافة بين العناصر
                            row.RelativeItem().AlignCenter().Text("التشغيل التجريبي ").FontFamily("Cairo")
                                .FontSize(12);
                        });
                        col.Spacing(5);
                        ...
                        col.Item().Scale(0.87f).Table(table =>
                        {
                            ...
                            foreach (var item in checkingItems.Where(x => x.Type == "التشغيل التجريبي "))
                            {
                                table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4)
                                    .Text(item.Item).FontFamily("Cairo").FontSize(9);
                        ...
                        {
                            row.Spacing(10); // المسافة بين العناصر
                            row.RelativeItem().AlignCenter().Text("اعتماد نهائي").FontFamily("Cairo")
                                  .FontSize(12);
                        });
                        col.Spacing(5);
                        ...
                        col.Item().Scale(0.87f).Table(table =>
                        {
                            ...
                            foreach (var item in checkingItems.Where(x => x.Type == "اعتماد نهائي"))
                            {
                                table.Cell().Border(1).PaddingVertical(1).PaddingHorizontal(4)
                                    .Text(item.Item).FontFamily("Cairo").FontSize(9);
    ...
    public async Task<IActionResult> GetElevatorReportPdf(int Id)
    {
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                {
                    // الشعار
                    col.Item().AlignCenter().Element(e => { e.Width(100).Height(50).Image(logoBytes).FitWidth(); });
                ...
                page.Content().ContentFromRightToLeft().Column(col =>
                {
                    ...
                    col.Item().AlignRight()
                        ...
                        .FontFamily("Cairo").FontSize(12);
                    col.Item().AlignRight().Text($"رقم الهاتف: {ElevatorReportDb.PhoneNum}").FontFamily("Cairo")
                        .FontSize(12);
                    ...
                    col.Item().AlignRight()
                        .Text(
                            $"مقاسات سيبس — العرض {ElevatorReportDb.resizableSquarewidth}    العمق {ElevatorReportDb.resizableSquareHeight}")
                        .FontFamily("Cairo").FontSize(12);

                    // Row 5: مقاسات فتحة الباب
                    col.Item().AlignRight()
                        .Text(
                            $"مقاسات فتحة الباب — العرض {ElevatorReportDb.directionWidth}   الطول {ElevatorReportDb.directionHeight}")
                        .FontFamily("Cairo").FontSize(12);

                    // Row 6: ثلاث صور في نفس الصف
                    col.Item().Row(row =>
                    {
                        row.RelativeItem().AlignCenter().Element(e => { e.Width(150).Height(150).Image(wellImage); });
                    ...
                    col.Item().AlignRight().Scale(0.85f).Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            ...
                            header.Cell().Border(1).Padding(2).AlignCenter().Text("الارتفاع").FontFamily("Cairo")
                                .Bold();
                        ...
                        {
                            table.Cell().Border(1).Padding(4).AlignCenter().Text(ElevatorReportDb.capinaHeight)
                                .FontFamily("Cairo").FontSize(9);
                        }
                        else
                        {
                            table.Cell().Border(1).Padding(4).AlignCenter().Text(ElevatorReportDb.capinaStatus)
                                .FontFamily("Cairo").FontSize(9);
                        }
...
: " ";
                            ...
                            {
                                table.Cell().Border(1).Padding(4).AlignCenter().Text(gargeheightList[i])
                                    .FontFamily("Cairo").FontSize(9);
                            }
                        ...
                        table.Cell().Border(1).Padding(4).AlignCenter().Text("حفره البئر").FontFamily("Cairo")
                            .FontSize(9);
                        if (ElevatorReportDb.foundationHeight != 0)
                        {
                            table.Cell().Border(1).Padding(4).AlignCenter().Text(ElevatorReportDb.foundationHeight)
                                .FontFamily("Cairo").FontSize(9);
                        }
                        else
                        {
                            table.Cell().Border(1).Padding(4).AlignCenter().Text(ElevatorReportDb.wellStatus)
                                .FontFamily("Cairo").FontSize(9);
                        }
                    ...
                    col.Item().AlignRight().Text($" الاعمال المطلوبة: {DeliveryReport.workRequied}").FontFamily("Cairo")
                        .FontSize(12);
                    ...
                    col.Item().AlignLeft().PaddingLeft(20).Text($"الفني المسئول: {ElevatorReportDb.TechName}")
                        .FontFamily("Cairo").FontSize(12);
                    col.Item().AlignLeft().PaddingLeft(20).Text($"مسئول المبيعات: {ElevatorReportDb.salesName}")
                        .FontFamily("Cairo").FontSize(12);
                });
    ...
    [RequestSizeLimit(20_000_000)] // 20 MB
    public async Task<IActionResult> EditEquipmentReport()
    {
        var form = await Request.ReadFormAsync();
        ...
        if(form.ContainsKey("existingImages"))
        {
            ...
            var allOldImages = await _db.ReportFiles
    ...
    [HttpPost("deletelevator/{id}")]
    public IActionResult deletelevator(int id)
    {
        var report = _db.LevatorReport.FirstOrDefault(x => x.Id == id);
        if(report!=null)
        {
            ...
            _db.Facade.RemoveRange(faceades);
            var reportImages = _db.LevatorImage.Where(x => x.LevatorReportId == id).ToList();
            _db.LevatorImage.RemoveRange(reportImages);
            _db.LevatorReport.Remove(report);
            _db.SaveChanges();
        }
    ...
    [HttpPost("deleteelevatorinspection/{id}")]
    public IActionResult deleteelevatorinspection(int id)
    {
        // مثال: إذا كنت تستخدم EF Core
        var report = _db.ElevatorInspectionReport.FirstOrDefault(x => x.Id == id);
        if(report!=null)
        {
    ...
    //[RequestSizeLimit(20_000_000)] // 20 MB
    //public async Task<IActionResult> PostLevatorReport()
    //{
    //    var form = await Request.ReadFormAsync();
    //    var currentYear = DateTime.Now.Year.ToString().Substring(2);
    ...
    [HttpGet("GetPagedLevatorReports")]
    public async Task<IActionResult> GetPagedLevatorReports(long userId , string? reportNumber , string? companyName ,
        string? techName , string? phoneNum , string? from , string? to , string? salesName , int page = 1 , int pageSize = 5)
    {
        var query = _db.LevatorReport .AsQueryable();
        ...
        // فلتر المبيعات
        if(!string.IsNullOrWhiteSpace(salesName))
            query=query.Where(x => x.salesperson.Contains(salesName));
        ...
        var imagesDb = await _db.LevatorImage
            .Where(i => reportlist.Select(x => x.Id).Contains(i.LevatorReportId))
            .ToListAsync();
        ...
        var reports = reportlist
            .Select(x => new SiteReportDto
            {
                ...
                Images = imagesDb != null
                    ? imagesDb
                        .Where(y => y.LevatorReportId == x.Id)
                        .Select(p => baseUrl + p.FilePath)
                        .ToList()
                    : null
            })
    ...
    public async Task<IActionResult> addLevatorReport()
    {
        ...
        else
        {
            ...
            if(last!=null)
            {
                var parts = last.ReportNumber.Split('/');
                if(parts.Length==2&&int.TryParse(parts [1] , out int num))
                    next=num+1;
            }

            report=new LevatorReport
            {
                ReportNumber=$"{year}/{next:D3}"
            };
        ...
        if(!string.IsNullOrEmpty(form ["facades"]))
        {
            ...
            for(int i = 0 ;i<facadeDtos.Count ;i++)
            {
                ...
                var facade = new Facade
                {
                    ...
                    TypeOfLand = dto.typeOfLand,
                    Height = dto.height,
                    heightWall = dto.heightWall,
                    Width = dto.width,
                    Max = dto.max,
        ...
        try
        {
            ...
            var reportDto = new ReportDetailsDto
            {
                ...
                facades = report.facades.Select(f => new FacadeDto
                {
                    ...
                    typeOfLand = f.TypeOfLand,
                    height = f.Height,
                    width = f.Width,
                    heightWall = f.heightWall,
                    max = f.Max,
                    notes = f.Notes,
                ...
                images = imagesDb.Select(i => new ImageDto
                {
                    id = i.Id,
                    fileName = i.FileName,
                    filePath = GetFullImagePath(i.FilePath)
                }).ToList()
            };
    ...
    public IActionResult GenerateLevatorReportPdf(int Id)
    {
     ...
     .Column(col =>
     {
                ...
                col.Item().Text("جدول الواجهات").FontSize(14).Bold().FontFamily("Cairo");
                    ...
                    foreach (var facade in report.facades)
                    {
                        col.Item().PaddingTop(5).Border(1).Padding(5).Column(fCol =>
                        {
                            ...
                            {
                                fCol.Item().PaddingTop(5).Text("السقالات التابعة:").FontFamily("Cairo").Bold();
                                fCol.Item().Table(table =>
                                {
                                    table.ColumnsDefinition(columns =>
                                    {
                                        columns.RelativeColumn(8);
                                    ...
                                    table.Header(header =>
                                    {
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("رقم").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("نوع الاستخدام").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("نوع المجموعة").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("تركيب المجموعه").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("نوع الصندوق").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("عرض الصندوق").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("الاطوال المطلوبه").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("عدد النقلات").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("طول الوايرات").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("طول كابل الكهرباء").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("يوجد مصدر كهرباء").FontFamily("Cairo").FontSize(8).Bold();
                                        header.Cell().Border(1).Padding(1).AlignCenter().Text("يوجد مصعد لرفع البضائع").FontFamily("Cairo").FontSize(8).Bold();
                                    });
                                    int i = 1;
                                    foreach (var scaffold in facade.Scaffolds)
                                    {
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(i.ToString()).FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.TypeOfUse ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.TypeOfGroup ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.SetGroup ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.TypeBox ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.WidthBox?.ToString() ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.HeightBox?.ToString() ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.NumberTransfers?.ToString() ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.Wirelength ?.ToString() ?? "-").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.ElectricWirelength ?.ToString() ?? "-").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.PowerSource ?? "").FontFamily("Cairo");
                                        table.Cell().Border(1).Padding(2).AlignCenter().Text(scaffold.Liftingoods ?? "").FontFamily("Cairo");
        ...
        var pdf = document.GeneratePdf();
        return File(pdf , "application/pdf" , $"LevatorReport_{report.ReportNumber}.pdf");
    }
    ...
    private string GetFullImagePath(string? filePath)
    {
        ...
        // إضافة الـ Base URL إذا كان مطلوباً
