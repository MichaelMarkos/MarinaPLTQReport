...
public class EquipmentReportController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllReports(int pageNumber = 1 , int pageSize = 10)
    {
        if(pageNumber<1)
            pageNumber=1;
        if(pageSize<1)
    ...
    new System.Text.Json.JsonSerializerOptions
    {
        PropertyNameCaseInsensitive = true
            ...
            foreach(var item in Items)
            {
                ...
                if(string.IsNullOrEmpty(item.Fault)&&string.IsNullOrEmpty(item.CorrectiveAction))
                    continue;
